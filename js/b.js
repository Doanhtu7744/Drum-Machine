document.addEventListener("DOMContentLoaded", function() {
    ...
    document.addEventListener('keydown', function(event) {
        ...
        const sound = currentSounds.find(sound => sound.keyTrigger === key);
        if (sound) {
            const audio = new Audio(sound.url);
            audio.volume = volumeSlider.value;
            audio.currentTime = 0; 
    ...
    bankButton.addEventListener('click', function() {
        if (powerOn) {
            ...
            currentSounds = (currentBank === 'HeaterKit') ? a : t;
    ...
    function playmusic() {
        ...
        const interval = setInterval(() => {
            ...
            const random = Math.floor(Math.random() * currentSounds.length);
            const randomSound = currentSounds[random];
            const audio = new Audio(randomSound.url);
            audio.currentTime = 0;
            audio.play();
            display.innerText = randomSound.id;
            count++;
        }, 500); 
    }
});
